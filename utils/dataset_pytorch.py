...
class BraceDataset(Dataset):
    def __init__(self, sequences_path, df, sample_length=900, max_length=None):
        assert (sample_length is None) != (
            max_length is None
        ), "Choose sample length or max length but not both"
        assert len(df) > 0
        self.df = df
        self.sequences = []
        self.clips = []
        ...
        for seq_t in tqdm(
            self.df.itertuples(), total=len(self.df), desc="Loading BRACE"
        ):
            ...
            for p in seq_clips:
                ...
                for cat in ("toprock", "footwork", "powermove"):
                    if cat in p.name:
                        assert (
                            clip_label is None
                        ), f"Trying to override clip labels for clip {p}"
        ...
        if max_length is not None:
            ...
            assert max_seq_length <= max_length, (
                f"Found a sequence whose length {max_seq_length} is > than the max "
                f"sequence allowed {max_length}. Adjust accordingly"
            )
        else:
            avg_seq_length = np.mean([x.shape[0] for x in self.sequences])
            print(
                f"Going to sample {sample_length} frames from each sequence. "
    ...
    def get_seq_clips(clip_paths_by_video, seq_t):
        return [
            p
            for (start, end), p in sorted(
                clip_paths_by_video[seq_t.video_id].items(), key=lambda kv: kv[0][0]
            )
            if seq_t.start_frame <= start <= seq_t.end_frame
            and seq_t.start_frame <= end <= seq_t.end_frame
        ]
    ...
    @staticmethod
    def load_clip(
        pose_path, img_ext=".png", broken_policy="skip_frame", policy_warning=False
    ):
        ...
        frame_numbers = [
            get_frame_number_from_id(video_id, f_id, img_ext=img_ext)
